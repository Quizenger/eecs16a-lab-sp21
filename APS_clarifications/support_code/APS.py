...
class APS:
    ...
    rawBinarySignal = None   # Raw binary signal, generated when microphone location is given
    ...
    def demodulate_signal(self, signal):
        ...
        print("mod signal len " + str(len(signal)))
        ...
        print("demod_real len pre conv " + str(len(demod_real)))
        print("demod_imaginary len pre conv " + str(len(demod_imaginary)))
        
        # Filter the signals
        demod_real = np.convolve(demod_real, self.LPF)#, mode='same')
        demod_imaginary = np.convolve(demod_imaginary, self.LPF)#, mode='same')
        print("demod_real len " + str(len(demod_real)))
        print("demod_imaginary len " + str(len(demod_imaginary)))
    ...
    def generate_raw_binary_signal(self, microphoneLocation, noise = False):
        
        self.microphoneLocation = microphoneLocation

        self.beaconList[0].generate_shifted_binary_signal(microphoneLocation, self.V_AIR) 
    
        self.rawBinarySignal = self.beaconList[0].shiftedBinarySignal
        
        for b in self.beaconList[1:]:
            
            b.generate_shifted_binary_signal(microphoneLocation,self.V_AIR)
            
            if b.offset == self.beaconList[0].offset:
                
                self.rawBinarySignal += np.roll(b.shiftedBinarySignal,1)
                
            else:
                
                self.rawBinarySignal = self.rawBinarySignal + b.shiftedBinarySignal
        
        if noise:
            print("Noise!!!!")
            noisySignal = self.add_random_noise(10)
            self.rawBinarySignal = np.roll(np.tile(noisySignal, 10),2000)
        else:
            print("No Noise!!!!")
            self.rawBinarySignal = np.roll(np.tile(self.rawBinarySignal, 10),2000)
...
#        print('TDOA', TDOA)
        ...
        print( "Weirdo Distances from Beacons : [%s]\n"%", ".join(["%0.4f" % d for d in dist_from_speakers]))
        
        # L2 NORMS
        dist_from_speakers = [np.linalg.norm([x - x_s, y - y_s]) for (x_s, y_s) in self.beaconsLocation]
        print( "Least Squares L2 Distances from Beacons : [%s]\n"%", ".join(["%0.4f" % d for d in dist_from_speakers]))
        
        # L2 NORMS
        x_m, y_m = self.microphoneLocation
        dist_from_speakers = [np.linalg.norm([x_m - x_s, y_m - y_s]) for (x_s, y_s) in self.beaconsLocation]
        print( "Actual L2 Distances from Beacons : [%s]\n"%", ".join(["%0.4f" % d for d in dist_from_speakers]))
...
#         self.simulation_testing(construct_system, least_squares, filename, isac=isac)
    ...
    class Beacons:
        ...
        shiftedBinarySignal = [] # shifted binary baseon the offset
        ...
        def generate_shifted_binary_signal(self, microphoneLocation, v_air):
            
            self.generate_offset(microphoneLocation, v_air)
            
            self.shiftedBinarySignal = np.roll(self.binarySignal, self.offset)
